...
async function parseAndCorrectJson(text: string, provider: string, modelId?: string): Promise<any> {
    ...
    try {
        ...
        const result = await generateChat({
            model: modelId || getModel(provider),
            provider,
            prompt,
...
async function runMultiStepAnalysis(
    ...
    if (customFunctions.size === 0) {
        ...
        return runSimpleAnalysis(vanillaCode, customCode, programName, manufacturingBranch, distributionBranch, provider, modelId);
    }
    ...
    for (let i = 0; i < functionsToAnalyze.length; i++) {
        ...
        try {
            ...
            const parsedJson = await parseAndCorrectJson(testCaseResponse.content, provider, modelId);
...
async function runSimpleAnalysis(vanillaCode: string, customCode: string, programName: string, manufacturingBranch: string, distributionBranch: string, provider: string, modelId: string): Promise<string> {
    const isFromScratch = !vanillaCode.trim();
...
export async function generateTestPlan(
    ...
    if (functionalSpecs && functionalSpecs.length > 0) {
        return runFunctionalSpecAnalysis(functionalSpecs, finalProgramName, manufacturingBranch, distributionBranch, module, provider, modelId, onProgress);
    }
    ...
    if (includeEnhancedAnalysis) {
        ...
        return runMultiStepAnalysis(vanillaCode, customCode, finalProgramName, manufacturingBranch, distributionBranch, provider, modelId, onProgress);
    } else {
        onProgress('Executando análise simples de código...');
        return runSimpleAnalysis(vanillaCode, customCode, finalProgramName, manufacturingBranch, distributionBranch, provider, modelId);
    }
}
